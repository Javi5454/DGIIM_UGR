 ...
 */
Matrix PageRank(Matrix & M, int max_iter, double threshold, int n){
    // Define matrix Pr and set to the initial values
    int nodes = M.columns();
    
    Matrix PR_ant(1, nodes, (1.0/nodes));
    Matrix PR_nuevo(PR_ant);
    int iter = 0;
    bool stable = false;
    ...
    while(iter < max_iter && !stable){
        ...
        for(int i = 0; i < PR_ant.columns(); i++){
            sum += abs(PR_nuevo(0,i) - PR_ant(0,i));
        }
...
Matrix computePageRank(Matrix & M, int max_iter, double threshold, int n = 10) {
    ...
    Matrix pagerank = PageRank(M,max_iter,threshold,n);
    cout << pagerank;
    ...
    while(ub < idx.size()){
        ...
        for(int j = lb; j < ub; j++){
            for(int k = j+1; k < ub; k++){
                brand1 = evs.at(idx.at(j).getPos()).getBrand();
                brand2 = evs.at(idx.at(k).getPos()).getBrand();
...
DataVector selectKbest(const Matrix & pr, int &k) {
    ...
    string *colLabels = new string [pr.columns()];
