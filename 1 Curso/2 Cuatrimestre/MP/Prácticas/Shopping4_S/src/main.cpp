...
int main(int argc, char** argv) {
    ...
    if (noargs) {
        ...
        if((ifilename == "error")||(n2Read == -1)||(indexby == "error")){
            errorBreak(ERROR_ARGUMENTS,"");
        }
    }
    ...
    EventSet originalES;
    bool error_test = originalES.read(ifile,n2Read);
    ...
    amountData.loadValues(money_field);
    ...
    EventSet to_index(rawFilterEvents(filteredES, reportby, fields[max_value]));
    
    Index traffic_index;
    traffic_index.build(to_index, OnBrand);
    ...
    cout << "Highest traffic choice (" << fields[max_value] << ") with " << max_traffic << " hits" << endl;
...
EventSet extractEventSet(EventSet es, Index indx) {
    ...
    for(int i=0; i < indx.size(); i++)
        result.add(es.at(indx.at(i).getPos()));
