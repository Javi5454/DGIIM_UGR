...
double AnsiTerminal::printVRuler(int x, int y, int height, int valmax, string labels[]) {
    int each = (int) std::round((height - 1)*1.0 / valmax);
    for (int i = 0; i <= height; i++) {
        setCursorXY(x, y + height - i);
...
AnsiTerminal &AnsiTerminal::printMinibarH(int x, int y, int width, double valmax, double level) {
    ...
    while (level > 0) {
        ...
        if (level >= 8) {
            setBackground(front);
            d+=1*valmax/(width+9);
            print(VBARS[8]).print(format("%-2.1f", d));
            bx++;
        } else {
            setBackground(back);
...
AnsiTerminal &AnsiTerminal::pressReturn() {
    cin.ignore();
...
void AnsiTerminal::close() {
    setCursorXY(1,_height-1).setText(white).setBackground(lightred).print("   RETURN   ");
    char c;
    cin >> c;
    pclose(fp);
    remove(_consoleinputfile.c_str());
