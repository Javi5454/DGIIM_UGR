...
/*	
	...
	agrupaciones que se presentan en varias circunscripciones. 
...
int main ()
{
	...
	// Resultado de la asignacion de escaños
	int escagnos [MAX_PROVINCIAS][MAX_PARTIDOS] = {0};
	
	
	// Resumen. Total de escaños conseguidos por cada partido
	int escagnos_partido [MAX_PARTIDOS] = {0};
	
	// Resumen. Total de escaños asignados en cada provincia
	int escagnos_provincia [MAX_PROVINCIAS] = {0};
	...
	*/
	...
	for (int prov=0; prov<num_provincias; prov++) {
		...
		cout << "CIRCUNSCRIPCION " << setw(2) << prov 
		     << " --> " << setw(12) << left << nombres_provincias[prov]; 	
		cout << "  Escagnos : " << setw(3) << right  
		     << resumen_provincias[prov].escagnos;  
	...
	// Linea de totales 
	cout << setw(48) << " " << "---" << setw(22) << " " << "--------" << endl; 
	cout << setw(40) << " " << "Total = " << setw(3) << total.escagnos;  
	cout << setw(14) << " " << "Total = " << setw(8) << total.censados << endl; 
	cout << endl; 
	...
	for (int prov=0; prov<num_provincias; prov++) {
		...
		// Escaños a repartir
		int num_escagnos = resumen_provincias[prov].escagnos; 
		...
		double tres_porciento = static_cast<int> (0.03 * votos_validos_prov);
				     
				     
		cout << "................................................." << endl;
		cout << endl; 		     		 		     		
		cout << "Calculando escagnos en provincia " << prov 
		...
		for (int part=0; part<num_partidos; part++) {
			...
			if (recuento[prov][part] > tres_porciento) {
				
		        distribucion[num_partidos_DHont].num_partido = part;
		        distribucion[num_partidos_DHont].votos = recuento[prov][part];
				...
				cout << "  Aceptado   partido " << setw(2) << part << " (" 
					 ...
					 << setw(8)  << recuento[prov][part] << " votos" << endl;
				#endif
			}
			else {
				#ifdef VERBOSE_DHONT
    	...
    	cout << "  Se consideran " << setw(3) << num_partidos_DHont 
		     << " partidos en " << nombres_provincias[prov] << endl; 
    	cout << endl; 
		...
		for (int part=0; part<num_partidos_DHont; part++) {
			...
			escagnos[prov][num_partido] = distribucion[part].escagnos;
		}
	    ...
	    for (int part = 0; part < num_partidos; part++) {
	    	...
	    	int num_escagnos_prov_part = escagnos[prov][part]; 
	...
	for (int part = 0; part < num_partidos; part++)
		...
		for (int prov=0; prov<num_provincias; prov++) {
			...
			escagnos_partido[part] += escagnos[prov][part]; 	
			suma_por_part += escagnos[prov][part];
		}
	...
	// Calcular el total de escaños asignados en cada provincia
    ...
    for (int prov = 0; prov < num_provincias; prov++)  
		...
		for (int part=0; part < num_partidos; part++) {
			...
			escagnos_provincia[prov] += escagnos[prov][part]; 
			suma_por_prov += escagnos[prov][part];	
		}
	
	// Comprobación de integridad 
	
	if (total.escagnos != suma_por_prov) {
		cout << endl;
		cout << "ERROR DE INTEGRIDAD (3)" << endl;
		cout << endl; 		
		exit (3); 
	}	
	
	// Deberíamos haber obtenido que 
	// 		total.escagnos == suma_por_part == suma_por_prov
	...
	cout << setw(23) << " ";
	for (int prov=0; prov<num_provincias; prov++) 
		cout << setw(8) << nombres_provincias[prov]; 
    ...
    for (int part = 0; part < num_partidos; part++) {
    	...
    	if (escagnos_partido[part] > 0) {
			...
			for (int prov=0; prov<num_provincias; prov++) {
				cout << setw(8)	 << escagnos[prov][part]; 
			}
			cout << setw(8)	<< escagnos_partido[part] << endl; 
	   	}
	}
	
	cout << setw(24) << " "  << setfill('-') << setw(72) << " " << endl;
	
	cout << setw(23) << setfill(' ') << " ";	
	for (int prov=0; prov<num_provincias; prov++) 
		cout << setw(8) << escagnos_provincia[prov]; 
