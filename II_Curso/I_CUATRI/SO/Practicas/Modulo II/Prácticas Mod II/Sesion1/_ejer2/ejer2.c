...
SOLEDAD RUIZ GOMEZ
ejer2.c
Práctica 1 actividad 3 ejercicio 4
*/
...
#define S_ISREG2(mode) ((mode & S_IFMT) == S_IFREG)		// macro que aplica la máscara al mode y comprueba si el resultado es S_IFREG
...
int main(int argc, char *argv[]) {
	...
	if(argc<2) {	// leemos los argumentos que nos han pasado
		printf("\nSintaxis de ejecucion: tarea2 [<nombre_archivo>]+\n\n");
		exit(EXIT_FAILURE);
	}

	for(i=1;i<argc;i++) {
		printf("%s: ", argv[i]);	// sacamos a pantalla el nombre del archivo i
		if(lstat(argv[i],&atributos) < 0) {
			...
			printf("%s\n",tipoArchivo);	// sacamos el tipo de archivo que sea
		}
	}
