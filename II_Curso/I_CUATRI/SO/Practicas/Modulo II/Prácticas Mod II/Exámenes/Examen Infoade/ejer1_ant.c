...
// los números i-nodo de todas y cada una de ellas
    ...
    if ((fifo[0]=open(ARCHIVO_FIFO,O_RDWR)) <0) {
    ...
    while ((file = readdir (directorio))!= NULL) {
        ...
        // Escribir en el cauce FIFO el numero de inodo
        char inodo[TAM];
        sprintf(inodo, "-%s: %ld \n",file->d_name, file->d_ino);

        printf ("\ninodo: %s", inodo);

        if( (write(fifo[0],inodo,strlen(inodo)+1)) != strlen(inodo)+1) {
...
// Creación del segundo hijo con fork ----------------------------------------------------------------------------------------------------
    ...
    if ((PID2 = fork()) < 0) {   // si hay error al crearlo
        ...
        // Empezamos por crear el archivo resultados.txt (o abrirlo y truncarlo)
        ...
        while(1) {
            ...
            if (write(fifo[1],mensaje,strlen(mensaje)) != strlen(mensaje)) {	// escribe lo guardado en buffer en el archivo fdr
